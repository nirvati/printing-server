...
(function(window, document, navigator, _ns) {
    ...
    _ns.getWindowLocationPath = function(path) {
        return window.location.protocol + "//" + window.location.host + "/" + path;
    };

    _ns.playSound = function(soundPath) {
        var soundURL;
        if (soundPath) {
            soundURL = _ns.getWindowLocationPath(soundPath);
            if (_ns.logger.isDebugEnabled()) {
                _ns.logger.debug('PlayAudio: ' + soundURL);
            }
